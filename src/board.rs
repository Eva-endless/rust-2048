...
fn load_best_score() -> i32 {
    let path = Path::new("best_score.txt");
    if path.exists() {
...
impl<'a> Board<'a> {
    ...
    fn save_state(&mut self) {
        ...
        let tiles_data: Vec<(i32, i32, i32)> = self.tiles.iter()
            .map(|t| (t.tile_x, t.tile_y, t.score))
            .collect();
    ...
    pub fn undo(&mut self) -> bool {
        if let Some(state) = self.history.pop() {
            self.tiles.clear();
            for &(x, y, score) in state.tiles.iter() {
                let mut tile = Tile::new(self.settings, score, x, y);
                tile.status = TileState::TileStatic;
    ...
    pub fn update(&mut self, dt: f64) {
        ...
        for i in 0..self.tiles.len() {
            ...
            for j in i+1..self.tiles.len() {
                ...
                let new_value = tile1.score + tile2.score;
                tiles_need_added.push(Tile::new_combined(self.settings, new_value, tile1.tile_x, tile1.tile_y));
                score_to_added += new_value;
                self.new_tile_values.push(new_value);
                break;
            }
        }
    ...
    pub fn render(&self, number_renderer: &NumberRenderer, c: &Context, gl: &mut GlGraphics) {
        let _ = number_renderer.render(
            self.score as u32,
            self.settings.score_rect[0] + self.settings.score_rect[2] / 2.0,
            self.settings.score_rect[1] + self.settings.score_rect[3] / 2.0,
            self.settings.score_rect[2],
            self.settings.text_light_color, c, gl);

        let _ = number_renderer.render(
            self.best_score as u32,
    ...
    pub fn get_tiles_info(&self) -> Vec<(i32, i32, u32)> {
        self.tiles.iter()
            .filter(|t| t.status == TileState::TileStatic)
            .map(|t| (t.tile_x, t.tile_y, t.score as u32))
            .collect()
    }

    pub fn has_new_tile_values(&self) -> bool {
        !self.new_tile_values.is_empty()
    }

    pub fn take_new_tile_values(&mut self) -> Vec<i32> {
        std::mem::take(&mut self.new_tile_values)
    }
    ...
    fn check_game_over(&mut self) {
        ...
        for i in 0..self.tiles.len() {
            ...
            for j in i+1..self.tiles.len() {
                let tile2 = &self.tiles[j];
                let is_adjacent = 
                    (tile1.tile_x == tile2.tile_x && (tile1.tile_y - tile2.tile_y).abs() == 1) ||
                    (tile1.tile_y == tile2.tile_y && (tile1.tile_x - tile2.tile_x).abs() == 1);
                
                if is_adjacent && tile1.score == tile2.score {
                    self.is_game_over = false;
                    return;
                }
            }
